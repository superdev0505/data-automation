...
class dbMySQL
{
  ...
  function bind($stm,$arr)
  {
    ...
    $refVal = refValues($params);
    if(!call_user_func_array('mysqli_stmt_bind_param',$refVal)) trigger_error(mysqli_error($this->conn),E_USER_WARNING);
    // if(!mysqli_stmt_bind_param($stm,$types,$params)) trigger_error(mysqli_error($this->conn),E_USER_WARNING);
  }
  
  function exec($stm,$trigger=TRUE)
  {
    if(!mysqli_stmt_execute($stm)) {
      // echo '{"error": "hkg error testing"}'; die();
